...
function login(credentials, users, callback) {
  ...
  users.findOne({ username: credentials.username }, (err, user) => {
    ...
    if (user) {
      ...
      bcrypt.hash(credentials.password, saltRounds, function(
        err,
        hashedPassword
      ) {
        if (err) return callback(err);
        ...
        login(credentials, users, (err, user) => {
          if (err && err.error === 'wrong-password') {
            return res.status(401).json({
              error:
                'Wrong password, if you tried to create an account then the username is already taken'
            });
          }
          if (err) {
...
app.post(
  ...
  (req, res) => {
    ...
    rooms.findOne({ name: message.room }, (err, room) => {
      ...
      if (room.isPrivate && !req.authenticatedUser.rooms.includes(room.name))
          ...
          users.find({ rooms: room.name }, (err, usersWithAccess) => {
            ...
            usersWithAccess.map(userWithAccess => {
              if (sockets.has(userWithAccess._id)) {
                sockets
                  .get(userWithAccess._id)
                  .forEach(socket => socket.emit('messages', message));
              }
            });
            res
              .status(201)
              .location(`/messages/${message._id}`)
              .json(message);
          });
        } else {
      ...
      rooms.insert(roomToCreate, (err, roomCreated) => {
        ...
        if (roomCreated.isPrivate) {
          users.update(
            ...
            { $addToSet: { rooms: roomCreated.name } },
            {},
            err => {
              if (err) return res.status(500).end();
              res
                .status(201)
                .location(`/rooms/${roomCreated.name}`)
                .json(roomCreated);
            }
          );
        } else {
...
app.post(
  ...
  (req, res) => {
    // TODO: remove duplication of access to a room
    rooms.findOne(
      { $or: [{ name: req.params.room }, { _id: req.params.room }] },
      (err, room) => {
        if (err) return res.status(500).end();
  ...
  rooms.find(
    {
      $or: [
        { isPrivate: false },
        { isPrivate: true, name: { $in: req.authenticatedUser.rooms } }
      ]
    },
    (err, rooms) => {
      res.status(200).json({
        rooms: rooms.map(({ name, topic, isPrivate }) => ({
          name,
          topic,
          isPrivate
        }))
...
app.get('/users', (req, res) => {
  users.find({}, (err, rooms) => {
    if (err) return res.status(500).end();
    res.status(200).json({
      users: rooms.map(({ _id, username, profile }) => ({
        username,
        profile,
        isConnected: sockets.has(_id)
      }))
    });
  });
});
...
app.put(
  ...
  (req, res) => {
    users.findOne(
      { $or: [{ username: req.params.user }, { _id: req.params.user }] },
      (err, user) => {
        if (!user) return res.status(404).end();
        if (req.authenticatedUser._id !== user._id)
...
io.on('connection', socket => {
  ...
  socket.on('disconnect', () => {
    ...
    const token = socket.handshake.query.token;
    let socketsForUser = sockets.get(token) || [];
    socketsForUser = socketsForUser.filter(
      socketForUser => socketForUser.id !== socket.id
    );
    sockets.set(token, socketsForUser);
...
rooms.insert(
  { name: 'main', topic: 'main', isPrivate: false },
  (err, _room) => {
    server.listen(4000, () => {
